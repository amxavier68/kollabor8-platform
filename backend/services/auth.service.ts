...
export class AuthService {
  ...
  async login(data: LoginRequest): Promise<{ user: IUser; tokens: AuthTokens; requires2FA?: boolean }> {
    ...
    const user = await User.findOne({ email: data.email }).select('+password +twoFactorSecret +twoFactorBackupCodes');
    if (!user) {
  ...
  async forgotPassword(email: string): Promise<void> {
    ...
    console.log('Reset URL would be: http://localhost:3000/reset-password?token=' + resetToken);
  }
